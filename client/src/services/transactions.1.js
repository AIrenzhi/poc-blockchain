...
const setBatcherPubkey = async () => {
  let res = await http.get('biz/transactions/info')
  batcherPublicKey = res.pubkey
}
setBatcherPubkey()

const requestPassword = () => {
  let password = '123456'
  return Promise.resolve().then(() => password)
}
...
const createPrivateKey = password => {
  privateKey = context.newRandomPrivateKey()
  signerPublicKey = context.getPublicKey(privateKey).asHex()
  ...
  return { privateKey, publicKey: signerPublicKey }
}
...
const submit = (payloads, wait = false) => {
  ...
  return Promise.resolve()
    ...
    .then(() => {
      ...
      return api.postBinary(`biz/transactions/submit${wait ? '?wait' : ''}`, txnList)
    })
}
export default{
  makePrivateKey,
